...
labels = pd.read_excel('/content/yolov5/labels.xlsx',index_col='번호') # 라벨링 파일 가져오기
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        label = None if hide_labels else (names[c] if hide_conf else f'{(labels.loc[int(names[c])]["이름"])} {conf:.2f}')
                        ...
                        # 이름일 떄 
                        # coordinate[0] = labels.loc[int(names[c])]["이름"]
